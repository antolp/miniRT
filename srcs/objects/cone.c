...
// 	v.co = vec_sub(ray->origin, cone->apex);
// 	v.d = ray->direction;
// 	v.tan2 = tan(cone->angle);
// 	v.tan2 = v.tan2 * v.tan2;

// 	v.d_dot_a = vec_dot(v.d, v.axis);
// 	v.co_dot_a = vec_dot(v.co, v.axis);

// 	v.d_perp = vec_sub(v.d, vec_mul(v.axis, v.d_dot_a));
// 	v.co_perp = vec_sub(v.co, vec_mul(v.axis, v.co_dot_a));

// 	q.a = vec_dot(v.d_perp, v.d_perp) - v.tan2 * (v.d_dot_a * v.d_dot_a);
// 	if (fabs(q.a) < 1e-8)
// 		return (false);

// 	q.b = 2.0 * (vec_dot(v.d_perp, v.co_perp) - v.tan2 * v.d_dot_a * v.co_dot_a);
// 	q.c = vec_dot(v.co_perp, v.co_perp) - v.tan2 * (v.co_dot_a * v.co_dot_a);
...
// 	candidate = -1.0;
// 	if (q.t0 > 1e-4 && (candidate < 0.0 || q.t0 < candidate))
// 		candidate = q.t0;
// 	if (q.t1 > 1e-4 && (candidate < 0.0 || q.t1 < candidate))
// 		candidate = q.t1;

// 	if (candidate < 0.0 || candidate > *t_hit)
// 		return (false);
